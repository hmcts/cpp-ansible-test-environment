...
# NOTE: Support python 2 & 3
#       there is other ways to fix this, using for instance past.buildins or other
...
class LookupModule(LookupBase):
    ...
    def _fetch_client_token(self, cafile, capath, url, data, cahostverify, skipverify):
        try:
            ...
            raise AnsibleError('Unable to retrieve personal token from vault due to AttributeError: %s' % (e))

        except urllib2.HTTPError as e:
            raise AnsibleError('Unable to retrieve personal token from vault due to HTTPError: %s' % (e))

        except Exception as e:
            raise AnsibleError('Unable to retrieve personal token from vault due to Exception: %s' % (e))
    ...
    def _fetch_remotely(self, cafile, capath, data, key, vault_token, url, cahostverify, skipverify):
        try:
            ...
            raise AnsibleError("Unable to read url '%s' with data '%s' from vault due to AttributeError: %s" % (request_url, data, e))

        except urllib2.HTTPError as e:
            raise AnsibleError("Unable to read url '%s' with data '%s' from vault due to HTTPError: %s response: %s" % (request_url, data, e, e.read() ))

        except Exception as e:
            raise AnsibleError("Unable to read url '%s' with data '%s' from vault due to Exception: %s" % (request_url, data, e))

        body = response.read()
        if response.headers.get('Content-Type') == 'application/json':
            body = json.loads(body)

        # Decode Python 3 strings / bytes
        if isinstance(body, six.binary_type):
          return body.decode('unicode-escape')
        else:
          return body
